...
const UpdateAdvertisment = (props) => {
	const currentAd = props.location.state.ad;
	const notification = useContext(NotificationContext);
	const inputRef = useRef();
	const [file, setFile] = useState();
	const [signrequest, setSignrequest] = useState();
	const [image, setImageUrl] = useState(currentAd.image);
	...
	const [adImage, setAdImage] = useState(currentAd.image);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const res = await axios.put(
				`${process.env.REACT_APP_BASE_URL}/api/advertisment/update`,
				body,
				config
			);
			if (res.status === 202) {
				notification.showNotification(
					'Advertisment updated successfully',
					false
				);
				setAdvertisment({
					title: '',
			...
			notification.showNotification(
				'cannot update the advertisment at the moment, Try again',
				true
			);
		}
	};
