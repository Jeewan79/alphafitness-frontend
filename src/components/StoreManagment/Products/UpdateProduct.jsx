...
const UpdateProduct = (props) => {
	const currentProduct = props.location.state.product;
	const [product, setProduct] = useState({
		name: currentProduct.name,
		smallDescription: currentProduct.smallDescription,
	...
	const [image, setImgurl] = useState(currentProduct.image);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const res = await axios.put(
				`${process.env.REACT_APP_BASE_URL}/api/product/update`,
				body,
				config
			);
			if (res.status === 200) {
