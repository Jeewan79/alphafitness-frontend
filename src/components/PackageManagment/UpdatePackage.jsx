...
const UpdatePackage = (props) => {
	const currentPackage = props.location.state.package;
	...
	const [packageDetails, setPackageDetails] = useState({
		...
		categoryId: currentPackage.category._id
	});
		...
		try {
			if (categoryId === '') {
				...
				const res = await axios.put(
					`${process.env.REACT_APP_BASE_URL}/api/package/update`,
					packageDetails,
					config
				);
			...
			<div className='row justify-content-center pb-5'>
				<div className='col-lg-6  col-md-6 col-sm-12 col-xs-12 background-border'>
					<div className='row m-2 text-center'>
						<h3>Update Package {currentPackage.name}</h3>
					</div>
					<hr />
